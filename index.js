...
// here I wrap everything from the module
// and I return what I would set to exports
...
var bigint = (function(){
  ...
  function Address(max, length){
    ...
    } else { //if ( (max) < pow(2, 25) ) {
      return new Uint32Array(length)
      // I thought this was a limit but it not always is
      //  } else if ( max >= pow(2, 25) ) {
  ...
  function Stack(type, size, silent){
    ...
    function extend(needed){
      ...
      if ( nl >= pow(2, adrs.BYTES_PER_ELEMENT) ) {
    ...
    return { alloc: alloc , free: free, heap: function(i){ return heap[i] }, adrs: function(i){return adrs[i]}
            , lastpointer: function(){ return next } }

  }

  var stack = Stack(Uint32Array, 32*1024*1024, false)
  ...
  function power_of_ten(i){
    if ( pot[i] !== undefined ) return pot[i]
...
// lets use it 
// these are from bn.js benchmark
// https://github.com/indutny/bn.js/blob/master/benchmarks/index.js
// honestly, i think these benchmarks are not very good, but I am not an expert
// and in most of the time, they probably measure something. 

var a = bigint.parse('012345678901234567890123456789012345678901234567890');
var b = bigint.parse('213509123601923760129376102397651203958123402314875');
var c = bigint.parse('3735928559');
var d = bigint.add(bigint.multiply(a, a), c);

var times = 100
var r = []
while ( times-- > 0 ) {
  r.push(bigint.divide(d, a))
}

// completely meaningless data, as it points to array indeces, not real bigints
