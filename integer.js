...
var stack = Stack(Uint32Array, 128*1024*1024, false)
...
function to_base10(bigint){
  var dec = []
  while( compare(bigint, tenM) >= 0 ) {
    var r = divide(bigint, tenM)
    var rem = r[1]
    var remp = adrs[rem]
...
function compare(A, B){
  ...
  return signA !== signB ? signA < signB 
...
function addition(A, B){
  ...
  var Rp = adrs[R]
//console.log(A,B)
//console.log(heap[Ap],heap[Bp],heap[Rp])
  if ( heap[Rp] > 2 ) {
    heap[Rp + 1] = signA
  }
...
var through_arrays = {
  add: arr_to_arr(addition)
, subtract: arr_to_arr(subtraction)
, multiply: arr_to_arr(multiplication)
, divide: function(a, b){ 
    var mark = alloc(2)
    var A = arr_to_int(a)
    var B = arr_to_int(b)
...
, compare_abs : arr_2to_primitive(compare_abs)
, compare : arr_2to_primitive(compare)
, equal : arr_2to_primitive(equal)
, parse : function(str){ 
    var mark = alloc(2)
    var I = parse(str)
    var r = int_to_arr(I)
